...
- `src/scene/composeDisplay.ts` — `composeDisplayObjects`: shape key / FFD / Path Deform / Follow Path / Fake Physics / Fake Flag 等を合成する共有変形パイプライン。**表示系は必ずこれを使う**（Viewport.tsx に古い重複コピーが残っているのは既知の負債。触るときは統合を検討）。
...
- **モーダル操作・配置待ちUIは Escape と右クリックの両方でキャンセル**できること。アプリ全体の規約。新しいモーダル操作を追加するときも必ず両対応する。
