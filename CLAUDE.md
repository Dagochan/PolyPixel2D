...
- `.pptd`フォーマット仕様（テクスチャペイント姉妹アプリ向け抜粋）: [PPTD_FORMAT.md](./PPTD_FORMAT.md) — Mesh/SceneObject/Material/UvIslandTransformやUV座標計算アルゴリズム、`PROJECT_VERSION`を変更したら更新する。

## 技術スタック

React + TypeScript + Vite / Three.js（OrthographicCameraの2Dビューポート）/ Zustand。
Vitestを軽量導入済み（2026-07-11〜）。対象は「入力→出力が決まる純粋関数」（`src/scene/animation.ts`のようなサンプリング・数式ロジック）のみで、UIやThree.jsの描画は引き続き手動検証。
検証は「型チェック + Vitest（純粋関数） + scratchpadスクリプト + previewでの実操作」で行う。

## コマンド・検証手順

- 開発サーバー: `preview_start` の `"dev"` 設定を使う（port 5183）。Bashで直接起動しない。
- 型チェック: `npx tsc --noEmit`（ビルドは `tsc -b && vite build`）
- 自動テスト: `npm test`（`vitest run`）。純粋関数のロジックを触ったら対応する`*.test.ts`を書く/更新する
...
5. `Mesh`/`SceneObject`のペイント関連フィールド（`material`, `mesh.faceColors`, `uvIslandTransforms`, `uvBaseVertices`, `islandZOrders`等）、UV座標計算アルゴリズム（`src/scene/uv.ts`）、`.pptd`のファイル構造・`PROJECT_VERSION`のいずれかを変更したら PPTD_FORMAT.md も更新する（テクスチャペイント姉妹アプリが参照する仕様書のため）。
